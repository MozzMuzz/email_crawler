---
title: "Scrape email addresses - R Version"
...
with single website: download all html of website, clean the HTML and look for regex matches for email. I use MaxDepth = 1 since a majority of websites disclose emails addresses within one level from the starting page. However, if you look for private email addresses, they yend to be burried deeped in the website, so it is advisable to set MaxDepth to 2 or 3 in that case (which increases runtime).
...
```{r}
Rcrawler(Website = "https://www.electricmobilityeurope.eu/", no_cores = 4, no_conn = 4, MaxDepth = 1, DIR = "./data/test")
...
#alternative regex: [[:alnum:].-_]+@[[:alnum:].-]+.[a-z]{2,4}
```
# Batch run
